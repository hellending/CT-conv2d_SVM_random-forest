...
clf = svm.SVC(gamma='auto',kernel='poly',C=0.8,random_state=2,probability=True,degree=3)
# 获取最终数据集
x_train,x_test,y_train,y_test = train_test_split(dataSet,label,test_size=0.2)
x_train = np.array(x_train,dtype=float).reshape(-1,x_train.shape[1]*x_train.shape[2]*x_train.shape[3])
x_test = np.array(x_test,dtype=float).reshape(-1,x_test.shape[1]*x_test.shape[2]*x_test.shape[3])
